...
def get_bitcoin_trade_volume(days, file_name):
    """

    :param days: int
        no of days : data neeeded
    :param file_name: str
        filePath
    :return: None
    """
    cg = CoinGeckoAPI()
    bitcoin_data = cg.get_coin_market_chart_by_id(id='bitcoin', vs_currency='usd', days=days, interval='daily')
    bitcoin_df = pd.DataFrame(bitcoin_data['total_volumes'][:-1], columns=['TimeStamp', 'Trade Volume'], )
    # convert the timestamp
    date_format = {datetime.utcfromtimestamp(i / 1000).strftime('%Y-%m-%d'): i for i in list(bitcoin_df['TimeStamp'])}
    bitcoin_df['Date'] = date_format
    bitcoin_df.to_csv(file_name, columns=['Date', 'Trade Volume'],index=False)
